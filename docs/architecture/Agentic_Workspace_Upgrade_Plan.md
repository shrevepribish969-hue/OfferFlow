...
出题决策采用「录用评分卡优先」：JD 决定面试官需要考察什么，简历决定优势、迁移能力和缺口需要如何验证，面经 RAG 只负责提供同类岗位的真实问法、高频模式和追问路径，不再要求每道题都从题库出发或强制绑定简历项目细节。

目标流程：

1. 先分析 JD 要求。
2. 根据公司、岗位、能力要求生成检索 query。
3. 建立岗位录用评分卡，并把简历证据标记为直接、可迁移、薄弱、缺失或风险。
4. 针对各录用维度从真实面经知识库检索问法和追问参考。
5. 结合简历证据与缺口生成覆盖岗位理解、动机、核心能力、迁移能力和风险验证的问题。
6. 在前端展示考察维度、面试官意图以及真实面经来源（如使用）。

产品价值：

> 面试准备先模拟面试官的录用判断，再用真实面经校准问题的表达和追问方式。
