...
import { FileText, Check, X, Download, RotateCw, ArrowRight, Sparkles, Target, Info, Eye, Loader2 } from 'lucide-react';
...
export const ResumeWorkView = ({ job, cardMsg, finalResumeCard, isProcessing, progressSteps, onComplete, onRegenerate, onGenerateResume }: ResumeWorkViewProps) => {
  const patches = cardMsg?.data?.optimization_patches || [];
  ...
  useEffect(() => {
    if (patches.length > 0) {
      const initialStates: Record<number, 'pending' | 'accepted' | 'rejected'> = {};
      patches.forEach((_: any, idx: number) => {
        initialStates[idx] = 'pending';
      });
      setPatchStates(initialStates);
    }
  }, [patches]);
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {!isProcessing && summary && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex items-start gap-3 shadow-sm">
              ...
              <div>
                <h3 className="font-bold text-indigo-900 text-sm mb-1">优化总结</h3>
                <p className="text-sm text-indigo-800/80 leading-relaxed">{summary.replace('Summary: ', '')}</p>
              </div>
            </div>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div className={`px-5 py-3 border-b flex justify-between items-center ${
                    isAccepted ? 'bg-green-50/50 border-green-100' : 'bg-slate-50/80 border-slate-100'
                  }`}>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white border border-slate-200 rounded-md shadow-sm">
                        <span className="font-bold text-xs text-slate-700">{patch.module || '模块'}</span>
                        <ArrowRight className="w-3 h-3 text-slate-400" />
                        <span className="font-bold text-xs text-indigo-600">{patch.target_name || '目标'}</span>
                      </div>
                      
                      {patch.gap_addressed && (
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 border border-amber-100 text-amber-700 rounded-md text-xs font-medium">
                          <Target className="w-3.5 h-3.5" />
                          针对: {patch.gap_addressed}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-5 grid grid-cols-1 gap-4 items-center bg-white relative">
                    ...
                    <div className="h-full">
                      ...
                      <div className="p-4 bg-red-50/30 rounded-lg border border-red-100 text-sm text-slate-600 h-full relative">
                        ...
                        <p className="whitespace-pre-wrap">{patch.original || '无内容'}</p>
                      </div>
                    </div>
                    ...
                    <div className="h-full">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-green-600 uppercase tracking-wider">优化后</span>
                      </div>
